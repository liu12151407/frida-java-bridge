...
function ensureClassInitialized (env, classRef) {
  console.log('skipping ensureClassInitialized()');
  /*
  const api = getApi();
  if (api.flavor !== 'art') {
    return;
...
function ensureArtKnowsHowToHandleHookedMethods () {
  console.log('skipping ensureArtKnowsHowToHandleHookedMethods()');
  return;
...
class ArtMethodMangler {
  ...
  replace (impl, isInstanceMethod, argTypes, vm, api) {
    ...
    console.log('hookedMethod:', hookedMethod, JSON.stringify(fetchArtMethod(hookedMethod)));
    console.log('replacementMethod:', replacementMethod, JSON.stringify(fetchArtMethod(replacementMethod)));

    const thunk = Memory.alloc(Process.pageSize);
    this.thunk = thunk;
    Memory.patchCode(thunk, 128, code => {
      const cw = new Arm64Writer(code, { pc: thunk });
      cw.putAddRegRegImm('x0', 'x19', 160); // x0 = &Thread::Current().managed_stack

      // Copy the top-level ManagedStack fragment here, along with a copy of LR.
      cw.putLdrRegRegOffset('x16', 'x0', 16); // x16 = x0->top_shadow_frame_
      cw.putPushRegReg('x16', 'lr');
      ...
      cw.putStrRegRegOffset('xzr', 'x0', 0);  // x0->tagged_top_quick_frame_.tagged_sp_ = 0
      ...
      cw.putLdrRegAddress('x16', api.artQuickGenericJniTrampoline);
      cw.putBlrReg('x16');

      // Restore the previous ManagedStack fragment, plus LR.
      cw.putAddRegRegImm('x0', 'x19', 160);
      cw.putPopRegReg('x16', 'x17');
      cw.putStrRegRegOffset('x16', 'x0', 0);
      cw.putStrRegRegOffset('x17', 'x0', 8);
      cw.putPopRegReg('x16', 'lr');
      cw.putStrRegRegOffset('x16', 'x0', 16);
    ...
    Interceptor.attach(origMethod.quickCode, function () {
      const { context } = this;
      console.log('*** Entering quick code:', JSON.stringify(context, null, 2));

      console.log('\n=== Stack: ===\n' + hexdump(context.sp, { ansi: true }));

      const thread = context.x19;
      const managedStack = thread.add(160);
      const taggedSp = managedStack.readPointer();
      const link = managedStack.add(pointerSize).readPointer();
      const topShadowFrame = managedStack.add(2 * pointerSize).readPointer();
      console.log('\n=== Thread::Current()->managed_stack ===');
      console.log(`tagged_top_quick_frame_.tagged_sp_: ${taggedSp}`);
      console.log(`link_: ${link}`);
      console.log(`top_shadow_frame_: ${topShadowFrame}`);

      const frame = taggedSp.readPointer();
      console.log(`\tframe: ${frame}`);
      const sp0 = frame.readPointer();
      console.log(`\tsp0: ${sp0}`);

      Stalker.follow(this.threadId, {
        transform(iterator) {
          let insn = iterator.next();
          iterator.putCallout(onBlockHit);
          do {
            iterator.keep();
  ...
  revert (vm) {
    patchArtMethod(this.hookedMethod, this.originalMethod, vm);
  }

  resolveTarget (wrapper, isInstanceMethod, env, api) {
    const target = cloneArtMethod(this.hookedMethod);
    patchArtMethod(target, this.originalMethod, env.vm);
    return target;
  }
}

function onBlockHit (context) {
  const { pc } = context;
  const insn = Instruction.parse(pc);
  console.log(`Hit ${DebugSymbol.fromAddress(pc).toString()}\t${insn.toString()}`);
  Thread.sleep(0.01);
}
...
function fixupArtQuickDeliverExceptionBug (api) {
  console.log('Skipping fixupArtQuickDeliverExceptionBug()');
  return;
