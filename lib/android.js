...
class ArtMethodMangler {
  ...
  replace (impl, isInstanceMethod, argTypes, vm, api) {
    ...
    console.log(`artMethodSpec=${JSON.stringify(artMethodSpec)}`);
    const artMethodOffset = artMethodSpec.offset;

    let origMethod = fetchArtMethod(this.methodId, vm);
    this.originalMethod = origMethod;

    if ((origMethod.accessFlags & kAccXposedHookedMethod) !== 0 && xposedIsSupported()) {
      const hookInfo = this.originalMethod.jniCode;
      this.hookedMethod = hookInfo.add(2 * pointerSize).readPointer();
      origMethod = fetchArtMethod(this.hookedMethod, vm);
      this.originalMethod = origMethod;
    }

    const { hookedMethod } = this;

    const replacementMethod = cloneArtMethod(hookedMethod);
    this.replacementMethod = replacementMethod;

    const originalFlags = hookedMethod.add(artMethodOffset.accessFlags).readU32();

    // Remove kAccFastInterpreterToInterpreterInvoke to disable use_fast_path in interpreter_common.h
    const replacementFlags = ((originalFlags & ~(kAccCriticalNative | kAccFastNative | kAccFastInterpreterToInterpreterInvoke)) |
        kAccNative) >>> 0;
    console.log(`originalFlags=0x${originalFlags.toString(16)}`);
    console.log(`replacementFlags=0x${replacementFlags.toString(16)}`);

    patchArtMethod(replacementMethod, {
      jniCode: impl,
    ...
    const quickCodeImpl = origMethod.quickCode;
    const quickCodeName = DebugSymbol.fromAddress(quickCodeImpl).toString();
    console.log(`quickCodeImpl=${quickCodeImpl} -- ${quickCodeName}`);
    if (quickCodeName.indexOf('libart.so!art_quick_to_interpreter_bridge') !== -1) {
      console.log('[A] Target will be run on interpreter');
      Interceptor.attach(Module.getExportByName('libart.so', 'artQuickToInterpreterBridge'), function (args) {
        const method = args[0];
        if (method.equals(hookedMethod)) {
          console.log(`artQuickToInterpreterBridge() method=${method} yay`);
          args[0] = replacementMethod;
        } else {
          console.log(`artQuickToInterpreterBridge() method=${method} nay`);
        }
      });

      Interceptor.attach(Module.getExportByName('libart.so', '_ZN3art11interpreter26EnterInterpreterFromInvokeEPNS_6ThreadEPNS_9ArtMethodENS_6ObjPtrINS_6mirror6ObjectEEEPjPNS_6JValueEb'), function (args) {
        const method = args[1];
        if (method.equals(hookedMethod)) {
          console.log(`EnterInterpreterFromInvoke() method=${method} yay`);
          args[1] = replacementMethod;
        } else {
          console.log(`EnterInterpreterFromInvoke() method=${method} nay`);
        }
      });

      Interceptor.attach(Module.getExportByName('libart.so', '_ZN3art11interpreter30EnterInterpreterFromDeoptimizeEPNS_6ThreadEPNS_11ShadowFrameEPNS_6JValueEbNS_24DeoptimizationMethodTypeE'), function (args) {
        console.log('EnterInterpreterFromDeoptimize()');
      });

      Interceptor.attach(Module.getExportByName('libart.so', '_ZN3art11interpreter30EnterInterpreterFromEntryPointEPNS_6ThreadERKNS_20CodeItemDataAccessorEPNS_11ShadowFrameE'), function (args) {
        console.log('EnterInterpreterFromEntryPoint()');
      });

      Interceptor.attach(Module.getExportByName('libart.so', '_ZN3art11interpreter33ArtInterpreterToInterpreterBridgeEPNS_6ThreadERKNS_20CodeItemDataAccessorEPNS_11ShadowFrameEPNS_6JValueE'), function (args) {
        console.log('ArtInterpreterToInterpreterBridge()');
      });
    } else {
      console.log('[B] Target has quick code!');
      const thunk = Memory.alloc(Process.pageSize);
      this.thunk = thunk;

      const threadOffsets = getArtThreadSpec(vm).offset;

      Memory.patchCode(thunk, 128, code => {
        const cw = new Arm64Writer(code, { pc: thunk });
        //cw.putPushRegReg('fp', 'lr');
        //cw.putMovRegReg('fp', 'sp');
        //cw.putBrkImm(42);

        cw.putAddRegRegImm('x0', 'x19', threadOffsets.managedStack); // x0 = &Thread::Current().managed_stack

        //// Copy the top-level ManagedStack fragment here.
        //cw.putLdrRegRegOffset('x16', 'x0', 16); // x16 = x0->top_shadow_frame_
        //cw.putPushRegReg('x16', 'x17');
        //cw.putLdrRegRegOffset('x17', 'x0', 8); // x17 = x0->link_
        //cw.putLdrRegRegOffset('x16', 'x0', 0); // x16 = x0->tagged_top_quick_frame_.tagged_sp_
        //cw.putPushRegReg('x16', 'x17');

        //// Clear the top and point it to the previous one that we copied.
        cw.putStrRegRegOffset('sp', 'x0', 0);  // x0->tagged_top_quick_frame_.tagged_sp_ = 0
        //cw.putMovRegReg('x16', 'sp');
        //cw.putStrRegRegOffset('x16', 'x0', 8);  // x0->link_ = previousFragment
        cw.putStrRegRegOffset('xzr', 'x0', 16); // x0->top_shadow_frame_ = nullptr

        // Swap out the ArtMethod * with our replacement's.
        cw.putLdrRegAddress('x0', replacementMethod);

        // Transition into our GenericJNI NativeCallback.
        cw.putLdrRegRegOffset('x16', 'x0', artMethodOffset.quickCode);
        cw.putBrReg('x16');

        // Restore the previous ManagedStack fragment.
        //cw.putAddRegRegImm('x18', 'x19', threadOffsets.managedStack);
        //cw.putPopRegReg('x16', 'x17');
        //cw.putStrRegRegOffset('x16', 'x18', 0);
        //cw.putStrRegRegOffset('x17', 'x18', 8);
        //cw.putPopRegReg('x16', 'x17');
        //cw.putStrRegRegOffset('x16', 'x18', 16);

        //cw.putPopRegReg('fp', 'lr');
        //cw.putRet();

        cw.flush();
        console.log(`Thunk size: ${cw.offset} bytes`);
      });

      console.log(`thunk=${thunk}`);

      disassemble(thunk, 23);

      Interceptor.replace(origMethod.quickCode, thunk);

      Interceptor.attach(origMethod.quickCode, function () {
        console.log('Called from:\n\t' + Thread.backtrace(this.context).map(DebugSymbol.fromAddress).join('\n\t'));
      });
    }

    console.log(`hookedMethod=${hookedMethod}`);
    console.log(`replacementMethod=${replacementMethod}`);
    console.log(`artQuickGenericJniTrampoline=${api.artQuickGenericJniTrampoline}`);
    ...
    Interceptor.attach(getOatQuickMethodHeaderImpl, {
      onEnter(args) {
        this.method = args[0];
      },
      onLeave(retval) {
        console.log(`GetOatQuickMethodHeader() method=${this.method} => ${retval}`);
      }
    });

    Interceptor.attach(Module.getExportByName('libart.so', '_ZNK3art11ClassLinker21IsQuickGenericJniStubEPKv'), function (args) {
      const linker = args[0];
      const entrypoint = args[1];
      console.log(`IsQuickGenericJniStub() linker=${linker} entrypoint=${entrypoint}`);
      Thread.sleep(0.1);
    });

    Interceptor.attach(Module.getExportByName('libart.so', '_ZNK3art20OatQuickMethodHeader7ToDexPcEPNS_9ArtMethodEmb'), function (args) {
      const header = args[0];
      const method = args[1];
      const pc = args[2];
      const abortOnFailure = args[3];
      console.log(`ToDexPc() header=${header} method=${method} pc=${pc} abort_on_failure=${abortOnFailure}`);
      Thread.sleep(0.01);
    });
    */
  }
  ...
  revert (vm) {
    Interceptor.revert(this.originalMethod.quickCode);
  }
...
function disassemble (address, n) {
  let cur = address;
  for (let i = 0; i !== n; i++) {
    let insn;
    try {
      insn = Instruction.parse(cur);
    } catch (e) {
      break;
    }
...
function fixupArtQuickDeliverExceptionBug (api) {
  ...
  Interceptor.attach(prettyMethod.impl, function (args) {
