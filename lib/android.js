...
class ArtMethodMangler {
  ...
  replace (impl, isInstanceMethod, argTypes, vm, api) {
    ...
    const map = new NativeFunction(
        Module.getExportByName('libart.so', '_ZN3art6MemMap29MapInternalArtLow4GBAllocatorEmiiil'),
        'pointer', ['pointer', 'int', 'int', 'int', 'pointer']);

    const slab = map(ptr(2 * Process.pageSize),
    ...
    const replacementMethod = slab.add(Process.pageSize);
    Memory.copy(replacementMethod, hookedMethod, artMethodSpec.size);
    ...
    const scratchSpace = slab.add(1024);
...
function fixupArtQuickDeliverExceptionBug (api) {
  ...
  Interceptor.attach(prettyMethod.impl, function (args) {
    ...
    //console.log('PrettyMethod! method=' + method);
    //Thread.sleep(0.1);
    if (method.isNull()) {
